...
/*void showfilter(const std::vector <helib::Ctxt> & vec, const helib::SecKey & sk){

    int nslots = sk.getContext().getEA().size();

    for(int i = 0; i < vec.size(); i++){

        helib::Ptxt<helib::BGV> res_ptxt(sk.getContext());
        sk.Decrypt(res_ptxt, vec[i]);

        for(int j = 0; j < nslots; j++){
            if(static_cast <long>(res_ptxt[j]) == 1)
                std::cout << i * nslots + j << ", ";
        }
    }
}*/
    ...
    void go() {
        ...
        for(int rnd = 0; rnd < this->RND_CNT; rnd++){
            ...
            **/
            ...
            /*std::vector <helib::Ctxt> q_mask(1, *server.QUERY_MASK);
            showfilter(q_mask, this->sk);
            std::cout << "query mask shown\n";
            showfilter(*server.filter, this->sk);*/
            ...
            **/
            for(int i = 0; i < QUERY_CNT; i++){
                ...
                //std::cout << "querying for " << rnd_query_element << ":\n";
            ...
            std::cout << "\n\nQueries finished\nStats:\n" <<
                        ...
                        "Number of (true) negatives: " << (QUERY_CNT - (false_positives + true_positives)) << '\n' << 
                        "Number of true positives: " << true_positives << '\n' << 
                        "Number of false positives: " << false_positives << '\n' << 
                        "Rate of false positives: " << (double)((false_positives + true_positives) ? (double)((double)false_positives / (double)(false_positives + true_positives)) : (double)0) << '\n';
...
The small test filter contains 10 elements and executes 100 queries\n\
The big test filter contains 60 elements and executes 200 queries\n\
small test [1] / big test [2] ?\n";
    ...
    if(test_type == 1){
        ...
        TestBloomFilter <3> * test = TestBloomFilter<3>::makeTest(test_round_cnt, 0.001, 100, 10); 
        test->go();
        ...
        TestBloomFilter <8> * test = TestBloomFilter<8>::makeTest(test_round_cnt, 0.001, 200, 60); 
        test->go();
