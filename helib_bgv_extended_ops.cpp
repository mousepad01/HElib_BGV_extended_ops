...
static helib::Ctxt __eq(std::vector <helib::Ctxt> & fst, std::vector <helib::Ctxt> & snd,
    ...
    if(j == 1){
        ...
        std::vector <long> constant_1(fst[0].getContext().getEA().size(), 1);
        helib::Ptxt <helib::BGV> ptxt_constant_1(fst[0].getContext(), constant_1);

        eq_dp[i][1].first = fst[i];
        eq_dp[i][1].first += snd[i];
        eq_dp[i][1].first += ptxt_constant_1;
    }
    else{
